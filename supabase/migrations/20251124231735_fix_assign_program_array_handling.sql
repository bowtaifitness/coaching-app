/*
  # Fix Array Handling in assign_program_from_intake

  1. Problem
    - Array subscript syntax error with uuid[]
    
  2. Changes
    - Use array_append instead of subscript notation
...
BEGIN
  ...
  IF v_intake_form.workout_frequency IS NOT NULL THEN
    IF v_intake_form.workout_frequency ILIKE '%3%' THEN v_days_per_week := 3;
    ELSIF v_intake_form.workout_frequency ILIKE '%4%' THEN v_days_per_week := 4;
    ELSIF v_intake_form.workout_frequency ILIKE '%5%' THEN v_days_per_week := 5;
    ELSIF v_intake_form.workout_frequency ILIKE '%6%' THEN v_days_per_week := 6;
    END IF;
  END IF;
  ...
  IF v_intake_form.equipment_access IS NOT NULL AND array_length(v_intake_form.equipment_access, 1) > 0 THEN
    IF 'Full Gym' = ANY(v_intake_form.equipment_access) THEN v_equipment_type := 'Full Gym';
    ELSIF 'Dumbbells' = ANY(v_intake_form.equipment_access) OR 'Dumbbell' = ANY(v_intake_form.equipment_access) THEN v_equipment_type := 'Dumbbell';
    ELSIF 'Resistance Bands' = ANY(v_intake_form.equipment_access) OR 'Bands' = ANY(v_intake_form.equipment_access) THEN v_equipment_type := 'Bands';
    END IF;
  END IF;
  ...
  WHERE program_type = 'standard' AND days_per_week = v_days_per_week AND title ILIKE '%' || v_equipment_type || '%'
  LIMIT 1;

  IF v_standard_program_id IS NULL THEN
    SELECT id INTO v_standard_program_id FROM workout_programs WHERE program_type = 'standard' AND days_per_week = v_days_per_week LIMIT 1;
  END IF;

  IF v_standard_program_id IS NULL THEN
    SELECT id INTO v_standard_program_id FROM workout_programs WHERE program_type = 'standard' LIMIT 1;
  END IF;
  ...
  -- Create workouts for each week/day
