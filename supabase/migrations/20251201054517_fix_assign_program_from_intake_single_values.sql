/*
  # Fix Program Assignment for Single Value Fields
  
  1. Changes
    - Update equipment_access handling from array to single text value
    - Update workout_frequency parsing to match new format ("2 days per week", "3 days per week", "4 days per week")
    
  2. Equipment Matching
    - Direct text comparison instead of array operations
    - Maps "Full Gym" → "Full Gym", "Dumbbells" → "Dumbbell", etc.
    
  3. Frequency Parsing
    - Extracts the number from "X days per week" format
...
BEGIN
  ...
  -- Parse workout frequency from new format "X days per week"
  v_days_per_week := 2;
  ...
  -- Determine equipment type from single text value
