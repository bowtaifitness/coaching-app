...
import { Plus, CreditCard as Edit3, Trash2, Calendar, Tag, Percent, Clock, Users, CheckCircle, XCircle, Settings, AlertCircle } from 'lucide-react';
...
const TrialManagement: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'trial' | 'promotions'>('trial');
  ...
  const [trialSaveStatus, setTrialSaveStatus] = useState<'idle' | 'success' | 'error'>('idle');
  ...
  const handleTrialDaysUpdate = async () => {
    setSavingTrial(true);
    setTrialSaveStatus('idle');

    try {
      const { error } = await supabase.rpc('update_default_trial_period', {
        new_trial_days: trialDays
      });

      if (error) throw error;

      setTrialSaveStatus('success');
      setTimeout(() => setTrialSaveStatus('idle'), 3000);
    } catch (error) {
      console.error('Error updating trial period:', error);
      setTrialSaveStatus('error');
      setTimeout(() => setTrialSaveStatus('idle'), 5000);
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Trial & Promotions Management</h1>
        <p className="text-gray-600 mt-2">Configure trial periods and manage promotional offers</p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="max-w-2xl">
            <div className="flex items-start mb-6">
              ...
              <div className="ml-4">
                <h2 className="text-xl font-bold text-gray-900 mb-2">Default Trial Period</h2>
                <p className="text-gray-600">
                  Set the default number of days for the free trial period. This applies to all new user signups.
                  Active promotions can override this setting for special offers.
                </p>
              </div>
            </div>
              ...
              <div className="flex items-start">
                ...
                <div className="text-sm text-blue-800">
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    <li>Changes only affect new signups going forward</li>
                    <li>Existing users keep their current trial periods</li>
                    <li>Active promotions will override this default</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-center space-x-4">
                  <input
                    ...
                    className="w-32 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg font-semibold"
                  />
              ...
              <div className="pt-4 border-t">
                ...
                {trialSaveStatus === 'success' && (
                  <div className="mt-4 flex items-center text-green-700 bg-green-50 border border-green-200 rounded-lg p-3">
                    <CheckCircle className="h-5 w-5 mr-2" />
                    <span className="font-medium">Trial period updated successfully!</span>
                  </div>
                )}

                {trialSaveStatus === 'error' && (
                  <div className="mt-4 flex items-center text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">
                    <XCircle className="h-5 w-5 mr-2" />
                    <span className="font-medium">Failed to update trial period. Please try again.</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mt-8 pt-8 border-t">
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600">Default Trial Length</p>
                    <p className="text-3xl font-bold text-gray-900 mt-1">{trialDays} days</p>
                  </div>
      ...
      {activeTab === 'promotions' && (
        <div>
          <div className="flex justify-between items-center mb-6">
            <p className="text-gray-600">Create and manage sales and promotional offers</p>
            <button
              onClick={() => {
                resetForm();
                setEditingPromotion(null);
